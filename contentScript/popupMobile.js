...
void async function () {
  ...
  // load html and icons
  const htmlText = await fetch(chrome.runtime.getURL("/contentScript/html/popupMobile.html")).then(response => response.text());
  const googleIcon = await fetch(chrome.runtime.getURL("/icons/google-translate-32.png")).then(response => response.blob());
  const yandexIcon = await fetch(chrome.runtime.getURL("/icons/yandex-translate-32.png")).then(response => response.blob());
  const bingIcon = await fetch(chrome.runtime.getURL("/icons/bing-translate-32.png")).then(response => response.blob());
  ...
  setInterval(() => {
    ...
    menuOptions.style.setProperty("--popup-height", `${popupElement.clientHeight}px`);
  }, 1000);
  ...
  function showPopup() {
    ...
    if (!rootElement.isConnected) {
      ...
      lastInteractionInterval = setInterval(() => {
        ...
        if (Date.now() - lastInteraction > 1000 * 8 && menuContainer.style.display !== "block" && serviceSelectorIsOpen === false && twpConfig.get("popupMobileKeepOnScren") === "no") {
          hidePopup();
        }
      }, 1000);
      updateInterface();
  ...
  function hidePopup(withoutAnimation = false) {
    ...
    if (rootElement.isConnected && !withoutAnimation) {
      ...
      const animation = mainElement.animate([{
        ...
        transform: `translateY(${twpConfig.get("popupMobilePosition") === "top" ? "-50px" : "50px"})`,
        opacity: "0"
      }], {
        duration: 200,
        iterations: 1
  ...
  // set text direction
  if (twpLang.isRtlLanguage(twpConfig.get("uiLanguage") || twpLang.fixUILanguageCode(chrome.i18n.getUILanguage()))) {
  ...
  function onMenuOptionClick(action) {
    ...
    if (action === "choose-another-language") {
      ...
      twpConfig.set("showTranslateSelectedButton", twpConfig.get("showTranslateSelectedButton") === "yes" ? "no" : "yes");
      ...
      window.open(chrome.runtime.getURL("/options/open-options.html") + `#!authorizationToOpenOptions=${authorizationToOpenOptions}`, "blank");
    } else if (action === "keep-on-screen") {
      twpConfig.set("popupMobileKeepOnScren", twpConfig.get("popupMobileKeepOnScren") === "yes" ? "no" : "yes");
    } else if (action === "change-position") {
      twpConfig.set("popupMobilePosition", twpConfig.get("popupMobilePosition") === "top" ? "bottom" : "top");
    }
    closeMenu();
    updateInterface();
  }
  ...
  function updateInterface() {
    ...
    if (twpConfig.get("showTranslateSelectedButton") === "yes") {
      shadowRoot.querySelector(`#menu-options [data-value="show-translate-selected-button"] [checkicon]`).textContent = "✔";
    } else {
      shadowRoot.querySelector(`#menu-options [data-value="show-translate-selected-button"] [checkicon]`).textContent = "";
    }
    ...
    if (twpConfig.get("neverTranslateSites").indexOf(tabHostName) !== -1) {
      shadowRoot.querySelector(`#menu-options [data-value="never-translate-this-site"] [checkicon]`).textContent = "✔";
    } else {
      shadowRoot.querySelector(`#menu-options [data-value="never-translate-this-site"] [checkicon]`).textContent = "";
    }
    ...
    from_to_element.textContent = twpI18n.getMessage("msgTranslateFromTo", [twpLang.codeToLanguage(tabLanguage), twpLang.codeToLanguage(twpConfig.get("targetLanguage"))]);

    // show or hide always-translate-from and never-translate-from
    if (tabLanguage === "und" || tabLanguage == twpConfig.get("targetLanguage")) {
      shadowRoot.querySelector(`#menu-options [data-value="always-translate-from"]`).style.display = "none";
      shadowRoot.querySelector(`#menu-options [data-value="never-translate-from"]`).style.display = "none";
    } else {
      shadowRoot.querySelector(`#menu-options [data-value="always-translate-from"]`).style.display = "flex";
      shadowRoot.querySelector(`#menu-options [data-value="never-translate-from"]`).style.display = "flex";
    }

    // update always-translate-from text
    shadowRoot.querySelector(`#menu-options [data-value="always-translate-from"] [data-i18n]`).textContent = twpI18n.getMessage("lblAlwaysTranslate", [twpLang.codeToLanguage(tabLanguage)]);

    // update always-translate-from checkicon
    if (twpConfig.get("alwaysTranslateLangs").indexOf(tabLanguage) !== -1) {
      shadowRoot.querySelector(`#menu-options [data-value="always-translate-from"] [checkicon]`).textContent = "✔";
    } else {
      shadowRoot.querySelector(`#menu-options [data-value="always-translate-from"] [checkicon]`).textContent = "";
    }
    ...
    if (twpConfig.get("neverTranslateLangs").indexOf(tabLanguage) !== -1) {
      shadowRoot.querySelector(`#menu-options [data-value="never-translate-from"] [checkicon]`).textContent = "✔";
    } else {
      shadowRoot.querySelector(`#menu-options [data-value="never-translate-from"] [checkicon]`).textContent = "";
    }
    ...
    if (twpConfig.get("popupMobileKeepOnScren") === "yes") {
      shadowRoot.querySelector(`#menu-options [data-value="keep-on-screen"] [checkicon]`).textContent = "✔";
    } else {
      shadowRoot.querySelector(`#menu-options [data-value="keep-on-screen"] [checkicon]`).textContent = "";
    }
  ...
  pageTranslator.onGetOriginalTabLanguage(function (_tabLanguage) {
    ...
    if (twpConfig.get("whenShowMobilePopup") !== "only-when-i-touch" && tabLanguage !== "und" && twpConfig.get("neverTranslateLangs").indexOf(tabLanguage) === -1 && twpConfig.get("neverTranslateSites").indexOf(tabHostName) === -1 && twpConfig.get("targetLanguage") !== tabLanguage) {
      showPopup();
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/popupMobile.js.map
