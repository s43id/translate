...
Promise.all([twpConfig.onReady(), getTabHostName()]).then(function (_) {
      ...
      if (piecesToTranslate && pageIsVisible) {
        (function () {
          ...
          if (piecesToTranslateNow.length > 0) {
            backgroundTranslateHTML(currentPageTranslatorService, currentSourceLanguage, currentTargetLanguage, piecesToTranslateNow.map(ptt => ptt.nodes.map(node => filterKeywordsInText(node.textContent, customDictionary, currentPageTranslatorService))), dontSortResults).then(results => {
          ...
          if (attributesToTranslateNow.length > 0) {
            backgroundTranslateText(currentPageTranslatorService, currentSourceLanguage, currentTargetLanguage, attributesToTranslateNow.map(ati => ati.original)).then(results => {
              if (pageLanguageState === "translated" && currentFooCount === fooCount) {
  ...
  function translatePageTitle() {
    ...
    backgroundTranslateSingleText(currentPageTranslatorService, currentSourceLanguage, currentTargetLanguage, originalPageTitle).then(result => {
      if (result) {
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/pageTranslator.js.map
