...
Promise.all([twpConfig.onReady(), getTabHostName()]).then(function (_) {
  ...
  fetch(chrome.runtime.getURL("/contentScript/css/showTranslated.css")).then(response => response.text()).then(response => styleTextContent = response).catch(e => console.error(e));
  ...
  function translateThisNode(node, usePrevNode = false) {
    ...
    let hasChildNodeBlock = function (node) {
      let foo = function (node) {
        const nodeName = node.nodeName.toLowerCase();
        if (htmlTagsInlineText.indexOf(nodeName) === -1 && htmlTagsInlineIgnore.indexOf(nodeName) === -1) {
          return true;
        }
    ...
    if (nodeName === "input" || nodeName === "textarea") {
      ...
      if (nodeName === "input" && (node.type === "button" || node.type === "submit")) {
        text = node.value;
    ...
    backgroundTranslateSingleText(currentTextTranslatorService, "auto", currentTargetLanguage, text).then(result => {
      if (!result) return;
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/showTranslated.js.map
