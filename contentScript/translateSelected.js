...
Promise.all([twpConfig.onReady(), getTabHostName()]).then(function (_) {
  ...
  function init() {
    ...
    eListenOriginal.onclick = async () => {
      let {
        lang,
        isReliable
      } = await detectTextLanguage(eOrigText.textContent);
  ...
  function update_eDivResult(result = "") {
    ...
    function onloaded() {
      ...
      eDivResult.style.left = Math.min(window.innerWidth - parseInt(getComputedStyle(eDivResult).width), left) + "px";
    }
    if (isCSSLoaded) {
      onloaded();
    } else {
  ...
  function translateNewInput() {
    ...
    const currentFooCount = fooCount;
    stopAudio();
    backgroundTranslateSingleText(currentTextTranslatorService, "auto", currentTargetLanguage, eOrigText.textContent).then(result => {
      if (currentFooCount !== fooCount) return;
  ...
  function getSelectionText() {
    ...
    if (activeElTagName == "textarea" || activeElTagName == "input" && /^(?:text|search)$/i.test(activeEl.type) && typeof activeEl.selectionStart == "number") {
  ...
  async function onUp(e) {
    ...
    if ((dontShowIfSelectedTextIsTargetLang == "yes" && detectedLanguage !== currentTargetLanguage || dontShowIfSelectedTextIsTargetLang != "yes") && (dontShowIfSelectedTextIsUnknown == "yes" && detectedLanguage !== "und" || dontShowIfSelectedTextIsUnknown != "yes") && (dontShowIfIsNotValidText != "yes" || isValidText(selectedText))) {
      init();
      if (platformInfo.isMobile.any) {
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/translateSelected.js.map
