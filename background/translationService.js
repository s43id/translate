...
const translationService = function () {
  ...
  class Utils {
    ...
    static escapeHTML(unsafe) {
      ...
      unsafe = unsafe.replaceAll(bingMarkFrontPart, "@-/629^*").replaceAll(bingMarkSecondPart, "^$537+*");
      unsafe = unsafe.replace(/\&/g, "&amp;").replace(/\</g, "&lt;").replace(/\>/g, "&gt;").replace(/\"/g, "&quot;").replace(/\'/g, "&#39;");
      unsafe = unsafe.replaceAll("@-/629^*", bingMarkFrontPart).replaceAll("^$537+*", bingMarkSecondPart);
      return unsafe;
    }
  ...
  class GoogleHelper_v2 {
    ...
    static async findAuth() {
      if (_authPromise._) return await _authPromise._;
      _authPromise._ = new Promise(resolve => {
        let updateGoogleAuth = false;
        if (_lastRequestAuthTime._) {
          const date = new Date();
          if (_translateAuth._) {
            date.setMinutes(date.getMinutes() - 20);
          } else if (_AuthNotFound._) {
            date.setMinutes(date.getMinutes() - 5);
          } else {
            date.setMinutes(date.getMinutes() - 1);
          }
          if (date.getTime() > _lastRequestAuthTime._) {
        ...
        if (updateGoogleAuth) {
          _lastRequestAuthTime._ = Date.now();
          const alternativeKey = new TextDecoder().decode(new Uint8Array([65, 73, 122, 97, 83, 121, 65, 84, 66, 88, 97, 106, 118, 122, 81, 76, 84, 68, 72, 69, 81, 98, 99, 112, 113, 48, 73, 104, 101, 48, 118, 87, 68, 72, 109, 79, 53, 50, 48]));
          const http = new XMLHttpRequest();
          http.open("GET", "https://translate.googleapis.com/_/translate_http/_/js/k=translate_http.tr.en_US.YusFYy3P_ro.O/am=AAg/d=1/exm=el_conf/ed=1/rs=AN8SPfq1Hb8iJRleQqQc8zhdzXmF9E56eQ/m=el_main");
          http.send();
          http.onload = e => {
            if (http.responseText && http.responseText.length > 1) {
              ...
              if (result && result.length === 2) {
                _translateAuth._ = result[1];
                _AuthNotFound._ = false;
              } else {
                _AuthNotFound._ = true;
                _translateAuth._ = alternativeKey;
              }
            } else {
              _AuthNotFound._ = true;
              _translateAuth._ = alternativeKey;
      ...
      _authPromise._.finally(() => {
        _authPromise._ = null;
      });
      return await _authPromise._;
    }
  }
  /** @type {number} */
  ...
  class YandexHelper {
    ...
    static async findSID() {
      if (_findPromise._) return await _findPromise._;
      _findPromise._ = new Promise(resolve => {
        let updateYandexSid = false;
        if (_lastRequestSidTime._) {
          const date = new Date();
          if (_translateSid._) {
            date.setMinutes(date.getMinutes() - 20);
          } else if (_SIDNotFound._) {
            date.setMinutes(date.getMinutes() - 5);
          } else {
            date.setMinutes(date.getMinutes() - 1);
          }
          if (date.getTime() > _lastRequestSidTime._) {
        ...
        if (updateYandexSid) {
          _lastRequestSidTime._ = Date.now();
          const http = new XMLHttpRequest();
          ...
          http.onload = e => {
            ...
            if (result && result[0] && result[0].length > 7) {
              _translateSid._ = result[0].substring(6);
              _SIDNotFound._ = false;
            } else {
              _SIDNotFound._ = true;
            }
            resolve();
      ...
      _findPromise._.finally(() => {
        _findPromise._ = null;
      });
      return await _findPromise._;
    }
  }
  /** @type {number} */
  ...
  class BingHelper {
     ...
     */
    static async findAuth() {
      if (_authPromise2._) return await _authPromise2._;
      _authPromise2._ = new Promise(resolve => {
        let updateBingAuth = false;
        if (_lastRequestAuthTime2._) {
          const date = new Date();
          if (_translateAuth2._) {
            date.setMinutes(date.getMinutes() - 8);
          } else if (_AuthNotFound2._) {
            date.setMinutes(date.getMinutes() - 5);
          } else {
            date.setMinutes(date.getMinutes() - 1);
          }
          if (date.getTime() > _lastRequestAuthTime2._) {
            updateBingAuth = true;
          }
        } else {
          updateBingAuth = true;
        ...
        if (updateBingAuth) {
          _lastRequestAuthTime2._ = Date.now();
          const http = new XMLHttpRequest();
          http.open("GET", "https://edge.microsoft.com/translate/auth");
          http.send();
          http.onload = e => {
            if (http.responseText && http.responseText.length > 1) {
              _translateAuth2._ = http.responseText;
              _AuthNotFound2._ = false;
            } else {
              _AuthNotFound2._ = true;
            }
            resolve();
          };
          http.onerror = http.onabort = http.ontimeout = e => {
      ...
      _authPromise2._.finally(() => {
        _authPromise2._ = null;
      });
      return await _authPromise2._;
    }
  }
  ...
  class Service {
    ...
    async makeRequest(sourceLanguage, targetLanguage, requests) {
      return await new Promise((resolve, reject) => {
        ...
        xhr.send(this.cbGetRequestBody ? this.cbGetRequestBody(sourceLanguage, targetLanguage, requests) : undefined);
      });
    }
  ...
  const googleService = new class extends Service {
    constructor() {
      super("google", "https://translate-pa.googleapis.com/v1/translateHtml", "POST", function cbTransformRequest(sourceArray) {
        ...
        //   );
        // }
        responseJson = response[0].map(/** @returns {Service_Single_Result_Response} */(/** @type {string} */value, /** @type {number} */index) => {
        ...
        if (this.DeepLTab) {
          chrome.tabs.get(this.DeepLTab.id, tab => {
            ...
            if (tab) {
              ...
              tabsCreate(`https://www.deepl.com/#!${targetLanguage}!#${encodeURIComponent(sourceArray2d[0][0])}`, tab => {
                this.DeepLTab = tab;
  ...
  const createLibreService = (url, apiKey) => {
    return new class extends Service {
      constructor() {
        super("libre", url, "POST", function cbTransformRequest(sourceArray) {
          ...
          return [result];
        }, null, function cbGetRequestBody(sourceLanguage, targetLanguage, requests) {
  ...
  const createDeeplFreeApiService = apiKey => {
    return new class extends Service {
      constructor() {
        super("deepl", "https://api-free.deepl.com/v2/translate", "POST", function cbTransformRequest(sourceArray) {
          return sourceArray[0];
        }, function cbParseResponse(response) {
          return [{
  ...
  const getSafeServiceByName = serviceName => {
    if (twpConfig.get("enabledServices").includes(serviceName) || twpConfig.get("customServices").find(cs => cs.name === serviceName)) {
  ...
  translationService.translateText = async (serviceName, sourceLanguage, targetLanguage, sourceArray, dontSaveInPersistentCache = false) => {
    ...
    const service = getSafeServiceByName(serviceName);
    return (await service.translate(sourceLanguage, targetLanguage, sourceArray.map(text => [text]), dontSaveInPersistentCache)).map(result => result[0]);
  };
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "translateHTML") {
      translationService.translateHTML(request.translationService, request.sourceLanguage, request.targetLanguage, request.sourceArray2d, dontSaveInPersistentCache, request.dontSortResults).then(results => sendResponse(results)).catch(e => {
        sendResponse();
        console.error(e);
      });
      return true;
    } else if (request.action === "translateText") {
      translationService.translateText(request.translationService, request.sourceLanguage, request.targetLanguage, request.sourceArray, dontSaveInPersistentCache).then(results => sendResponse(results)).catch(e => {
        sendResponse();
        console.error(e);
      });
      return true;
    } else if (request.action === "translateSingleText") {
      translationService.translateSingleText(request.translationService, request.sourceLanguage, request.targetLanguage, request.source, dontSaveInPersistentCache).then(results => sendResponse(results)).catch(e => {
        sendResponse();
        console.error(e);
      });
      return true;
      ...
      serviceList.set("libre", createLibreService(request.libre.url, request.libre.apiKey));
    } else if (request.action === "removeLibreService") {
      serviceList.delete("libre");
    } else if (request.action === "createDeeplFreeApiService") {
      serviceList.set("deepl", createDeeplFreeApiService(request.deepl_freeapi.apiKey));
    } else if (request.action === "removeDeeplFreeApiService") {
      serviceList.set("deepl", /** @type {Service} */ /** @type {?} */deeplService);
    }
  });
  twpConfig.onReady(function () {
    var _proxyServers$google;
    ...
    if (twpConfig.get("customServices").find(cs => cs.name === "deepl_freeapi")) {
      const deepl_freeapi = twpConfig.get("customServices").find(cs => cs.name === "deepl_freeapi");
      serviceList.set("deepl", createDeeplFreeApiService(deepl_freeapi.apiKey));
    }
    const proxyServers = twpConfig.get("proxyServers");
    if (proxyServers !== null && proxyServers !== void 0 && (_proxyServers$google = proxyServers.google) !== null && _proxyServers$google !== void 0 && _proxyServers$google.translateServer) {
  ...
  twpConfig.onChanged((name, newValue) => {
    if (name === "proxyServers") {
      var _newValue$google;
      if (newValue !== null && newValue !== void 0 && (_newValue$google = newValue.google) !== null && _newValue$google !== void 0 && _newValue$google.translateServer) {
        const url = new URL(googleService.baseURL);
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/translationService.js.map
