...
const translationCache = function (_Cache) {
  ...
  class Cache {
    ...
    async query(originalText) {
      ...
      translation = await _assertClassBrand(_Cache_brand, this, _queryInDB).call(this, hash);
    ...
    async add(originalText, translatedText, detectedLanguage = "und") {
      const hash = await Utils.stringToSHA1String(originalText);
      return await _assertClassBrand(_Cache_brand, this, _addInDb).call(this, {
        originalText,
        translatedText,
        detectedLanguage,
        key: hash
  ...
  _Cache = Cache;
  async function _queryInDB(origTextHash) {
    return await new Promise((resolve, reject) => {
      if (!this.db) return reject();
      const storageName = _Cache.getCacheStorageName();
  ...
  var _CacheList_brand = /*#__PURE__*/new WeakSet();
  class CacheList {
    /**
    ...
    constructor() {
       ...
       */
      _classPrivateMethodInitSpec(this, _CacheList_brand);
      /** @type {Map<string, Cache>} */
      this.list = new Map();
      try {
        _assertClassBrand(_CacheList_brand, this, _openCacheList).call(this);
      } catch (e) {
        console.error(e);
      }
    }
    ...
    async getCache(translationService, sourceLanguage, targetLanguage) {
      ...
      if (cache) {
        ...
        return await _assertClassBrand(_CacheList_brand, this, _createCache).call(this, translationService, sourceLanguage, targetLanguage);
      }
    }
    ...
    async deleteAll() {
      try {
        ...
        this.list.clear();
        const dbnames = await _assertClassBrand(_CacheList_brand, this, _getAllDBNames).call(this);
  ...
  function _openCacheList() {
    ...
    request.onsuccess = event => {
      ...
      this.list.forEach((cache, key) => {
        _assertClassBrand(_CacheList_brand, this, _addCacheList).call(this, key);
      });
    };
    request.onerror = request.onblocked = event => {
   ...
   * @returns {Promise<Cache>}
   */
  async function _createCache(translationService, sourceLanguage, targetLanguage) {
    const cache = new Cache(translationService, sourceLanguage, targetLanguage);
    _assertClassBrand(_CacheList_brand, this, _addCache).call(this, translationService, sourceLanguage, targetLanguage, cache);
    try {
      await cache.start();
    } catch (e) {
      console.error(e);
  ...
  function _addCache(translationService, sourceLanguage, targetLanguage, cache) {
    const dbName = Cache.getDataBaseName(translationService, sourceLanguage, targetLanguage);
    this.list.set(dbName, cache);
    try {
      _assertClassBrand(_CacheList_brand, this, _addCacheList).call(this, dbName);
    } catch {}
  }
  /**
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/translationCache.js.map
