...
const textToSpeech = function () {
  ...
  class BingHelper {
    ...
    static async findAuth() {
      if (_promise._) return await _promise._;
      _promise._ = new Promise(resolve => {
        let updateBingAuth = false;
        if (_lastRequestTime._) {
          const date = new Date();
          if (_IG._) {
          ...
          if (date.getTime() > _lastRequestTime._) {
            updateBingAuth = true;
          }
        } else {
          updateBingAuth = true;
  ...
  class Service {
     ...
     */
    constructor(serviceName, baseURL, xhrMethod, cbGetExtraParameters, cbGetRequestBody = null) {
  ...
  const bingService = new Service("bing", "https://www.bing.com/tfettts?isVertical=1", "POST", function getExtraParameters(text, targetLanguage) {
    return `&&IG=${encodeURIComponent(BingHelper.IG)}&IID=${encodeURIComponent(BingHelper.IID)}.${"1"}`;
  }, function getRequestBody(text, targetLanguage) {
    const languageData = BingHelper.getLanguageData(targetLanguage);
    const domParser = new DOMParser();
    const doc = domParser.parseFromString(`<speak version='1.0' xml:lang=''><voice xml:lang='' xml:gender='' name=''><prosody rate='-20.00%'></prosody></voice></speak>`, "text/xml");
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "textToSpeech") {
      if (twpConfig.get("textToSpeechService") === "bing") {
        bingService.textToSpeech(request.text, request.targetLanguage).finally(() => {
          sendResponse();
        });
      } else {
  ...
  twpConfig.onReady(async () => {
    ...
    twpConfig.onChanged((name, newvalue) => {
      if (name === "ttsSpeed") {
        ...
        if (proxyServers !== null && proxyServers !== void 0 && (_proxyServers$google = proxyServers.google) !== null && _proxyServers$google !== void 0 && _proxyServers$google.ttsServer) {
    ...
    if (proxyServers !== null && proxyServers !== void 0 && (_proxyServers$google2 = proxyServers.google) !== null && _proxyServers$google2 !== void 0 && _proxyServers$google2.ttsServer) {
      const url = new URL(googleService.baseURL);
      url.host = proxyServers.google.ttsServer;
      googleService.baseURL = url.toString();
    }
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/textToSpeech.js.map
