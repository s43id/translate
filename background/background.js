...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "getMainFramePageLanguageState") {
    ...
    try {
      if (platformInfo.isMobile.any && !platformInfo.isFirefox || platformInfo.isDesktop.any && platformInfo.isOpera) {
        chrome.tabs.sendMessage(sender.tab.id, {
...
function updateContextMenu(pageLanguageState = "original") {
  ...
  if (pageLanguageState === "translated") {
    ...
    contextMenuTitle = twpI18n.getMessage("msgTranslateFor", twpLang.codeToLanguage(targetLanguage));
  }
  if (typeof chrome.contextMenus != "undefined") {
    chrome.contextMenus.remove("translate-web-page", checkedLastError);
    chrome.contextMenus.remove("translate-restore-this-frame", checkedLastError);
...
if (typeof chrome.contextMenus !== "undefined") {
  ...
  chrome.contextMenus.onClicked.addListener((info, tab) => {
    if (info.menuItemId == "translate-web-page") {
      ...
      if (chrome.pageAction && chrome.pageAction.openPopup && (!tab || !tabHasContentScript[tab.id] || tab.isInReaderMode)) {
        chrome.pageAction.setPopup({
          popup: "popup/popup-translate-text.html#text=" + encodeURIComponent(info.selectionText),
          tabId: (tab === null || tab === void 0 ? void 0 : tab.id) || currentTabId
        });
        chrome.pageAction.openPopup();
        resetPageAction((tab === null || tab === void 0 ? void 0 : tab.id) || currentTabId);
      } else {
        ...
        if (pageLanguageState === "translated" && twpConfig.get("popupBlueWhenSiteIsTranslated") === "yes") {
          ...
          if (isFirefoxAlpenglowTheme) {
            if (darkMode || incognito) {
              svg64 = btoa(svg64.replace(/\$\(fill\)\;/g, "hsla(157, 100%, 66%, 1)"));
            } else {
              svg64 = btoa(svg64.replace(/\$\(fill\)\;/g, "hsla(180, 100%, 32%, 1)"));
            }
          } else {
            if (themeColorFrame && themeColorToolbar && themeColorToolbarField) {
              try {
                darkMode = isDarkColor(standardize_color(themeColorFrame, themeColorToolbar, themeColorToolbarField));
              } catch (e) {
                console.error(e);
              }
            } else if (themeColorFieldText) {
              try {
                darkMode = !isDarkColor(standardize_color(themeColorFieldText, themeColorFieldText, themeColorFieldText));
              } catch (e) {
                console.error(e);
              }
            }
          ...
          if (isFirefoxAlpenglowTheme) {
            if (darkMode || incognito) {
              svg64 = btoa(svg64.replace(/\$\(fill\)\;/g, "hsla(255, 100%, 94%, 1)"));
            } else {
              svg64 = btoa(svg64.replace(/\$\(fill\)\;/g, "hsla(261, 53%, 15%, 1)"));
            }
          } else {
            if (themeColorFieldText) {
              svg64 = btoa(svg64.replace(/\$\(fill\)\;/g, themeColorFieldText));
            } else if (!isUsingTheme && (darkMode || incognito)) {
              svg64 = btoa(svg64.replace(/\$\(fill\)\;/g, "rgb(251, 251, 254)"));
            } else {
  ...
  chrome.commands.onCommand.addListener(command => {
    if (command === "hotkey-toggle-translation") {
      ...
      chrome.tabs.query({
        ...
        sendTranslatePageMessage(tabs[0].id, twpConfig.get("targetLanguages")[0]);
      });
      ...
      chrome.tabs.query({
        ...
        sendTranslatePageMessage(tabs[0].id, twpConfig.get("targetLanguages")[1]);
      });
      ...
      chrome.tabs.query({
        ...
        sendTranslatePageMessage(tabs[0].id, twpConfig.get("targetLanguages")[2]);
      });
...
chrome.runtime.onUpdateAvailable.addListener(details => {
  ...
  chrome.tabs.query({}, tabs => {
    ...
    tabs.forEach(tab => {
      cleanUpsPromises.push(new Promise(resolve => {
        chrome.tabs.sendMessage(tab.id, {
          action: "cleanUp"
        }, resolve);
      }));
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/background.js.map
