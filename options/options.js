...
setTimeout(() => {
  fetch("./release-notes/en.html").then(response => response.text()).then(responseText => {
    ...
    document.getElementById("_msgHasBeenUpdated").textContent = twpI18n.getMessage("msgHasBeenUpdated");
    document.getElementById("_msgHasBeenUpdated").innerHTML = document.getElementById("_msgHasBeenUpdated").textContent.replace("#EXTENSION_NAME#", "<b>" + chrome.runtime.getManifest().name + "</b>").replace("#EXTENSION_VERSION#", "<b>" + chrome.runtime.getManifest().version + "</b>");
    document.getElementById("_donationText").textContent = twpI18n.getMessage("donationText");
    document.getElementById("_donatewithpaypal").textContent = twpI18n.getMessage("donatewithpaypal");
    document.getElementById("_donationRecipient").textContent = twpI18n.getMessage("msgDonationRecipient");
    document.getElementById("_donationRecipient").innerHTML = document.getElementById("_donationRecipient").textContent.replace("#EXTENSION_NAME#", "<b>" + chrome.runtime.getManifest().name + "</b>");
...
twpConfig.onReady().then(() => {
  ...
  function hashchange() {
    ...
    const divs = [$("#languages"), $("#sites"), $("#translations"), $("#style"), $("#hotkeys"), $("#privacy"), $("#storage"), $("#others"), $("#experimental"), $("#donation"), $("#release_notes")];
  ...
  $("#useAlternativeService").value = twpConfig.get("useAlternativeService");
  {
    ...
    const updateServiceSelector = enabledServices => {
      document.querySelectorAll("#pageTranslatorService option").forEach(option => option.setAttribute("hidden", ""));
      document.querySelectorAll("#textTranslatorService option").forEach(option => option.setAttribute("hidden", ""));
    ...
    const servicesInfo = [{
      selector: "#btnEnableGoogle",
      svName: "google"
    }, {
      selector: "#btnEnableBing",
      svName: "bing"
    }, {
      selector: "#btnEnableYandex",
      svName: "yandex"
    }, {
      selector: "#btnEnableDeepL",
      svName: "deepl"
    }];
    servicesInfo.forEach(svInfo => {
      $(svInfo.selector).oninput = e => {
        ...
        if (enabledCount === 0 || enabledCount === 1 && $("#btnEnableDeepL").checked) {
        ...
        chrome.runtime.sendMessage({
          ...
          serviceNames: pageTranslationServices.filter(svName => !enabledServices.includes(svName)),
      ...
      $(svInfo.selector).checked = twpConfig.get("enabledServices").indexOf(svInfo.svName) === -1 ? false : true;
  ...
  $("#addDeepL").onclick = async () => {
    ...
    try {
      ...
      if (response) {
        const customServices = twpConfig.get("customServices");
        const index = customServices.findIndex(cs => cs.name === "deepl_freeapi");
        if (index !== -1) {
          customServices.splice(index, 1);
        }
  ...
  $("#addGoogleProxy").onclick = e => {
    try {
      const inputTranslationServer = $("#googleTranslateProxyServer").value.trim();
      const inputTtsServer = $("#googleTtsProxyServer").value.trim();
      const translateServer = inputTranslationServer ? new URL("https://" + inputTranslationServer).host : null;
      const ttsServer = inputTtsServer ? new URL("https://" + inputTtsServer).host : null;
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/options.js.map
