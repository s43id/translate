...
const twpConfig = function () {
  ...
  const defaultConfig = {
    ...
    // when-necessary only-when-i-touch always-show
    useOldPopup: "yes",
    darkMode: "auto",
    // auto yes no
    ...
    hotkeys: {},
    // Hotkeys are obtained from the manifest file
  ...
  chrome.i18n.getAcceptLanguages(acceptedLanguages => {
    chrome.storage.local.get(null, onGot => {
      ...
      for (const lang in defaultTargetLanguages) {
        if (config.targetLanguages.length >= 3) break;
        if (config.targetLanguages.indexOf(defaultTargetLanguages[lang]) === -1) {
  ...
  twpConfig.swapPageTranslationService = function () {
    ...
    const pageEnabledServices = twpConfig.get("enabledServices").filter(svName => pageTranslationServices.includes(svName));
    const index = pageEnabledServices.indexOf(twpConfig.get("pageTranslatorService"));
    if (index !== -1) {
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/config.js.map
