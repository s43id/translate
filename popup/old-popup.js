...
twpConfig.onReady().then(() => twpI18n.updateUiMessages()).then(() => {
  ...
  const divShowTranslateSelectedButton = document.getElementById("divShowTranslateSelectedButton");
  const cbShowTranslateSelectedButton = document.getElementById("cbShowTranslateSelectedButton");
  ...
  // fill language list
  {
    ...
    lblTranslate.textContent = twpI18n.getMessage("lblTranslatePageInto", langs[twpConfig.get("targetLanguage")] || twpConfig.get("targetLanguage"));
    lblTranslated.textContent = twpI18n.getMessage("lblPageTranslateInto", langs[twpConfig.get("targetLanguage")] || twpConfig.get("targetLanguage"));
    const langsSorted = [];
  ...
  function updateInterface() {
    if (currentPageTranslatorService == "yandex") {
      $("#btnOptions option[value='translateInExternalSite']").textContent = twpI18n.getMessage("msgOpenOnYandexTranslator");
      $("#iconTranslate").setAttribute("src", "/icons/yandex-translate-32.png");
    } else if (currentPageTranslatorService == "bing") {
      $("#btnOptions option[value='translateInExternalSite']").textContent = twpI18n.getMessage("btnOpenOnGoogleTranslate");
    ...
    if (originalTabLanguage !== "und") {
      ...
      $("#lblAlwaysTranslateThisLang").textContent = twpI18n.getMessage("lblAlwaysTranslate", twpLang.codeToLanguage(originalTabLanguage));
      const translatedWhenHoveringThisLangText = twpI18n.getMessage("lblShowTranslatedWhenHoveringThisLang", twpLang.codeToLanguage(originalTabLanguage));
      ...
      const neverTranslateLangText = twpI18n.getMessage("btnNeverTranslateThisLanguage");
    ...
    if (showSelectTargetLanguage) {
      ...
      switch (currentPageLanguageState) {
        ...
        default:
          ...
          showAlwaysTranslateCheckbox ? void 0 : cbAlwaysTranslate.setAttribute("disabled", "disabled");
          btnTranslate.style.display = "inline";
          btnRestore.style.display = "none";
          btnTryAgain.style.display = "none";
          btnOptionsDiv.style.display = "inline";
      ...
      switch (btnOptions.value) {
        ...
        case "translateInExternalSite":
          chrome.tabs.query({
            ...
            if (currentPageTranslatorService === "yandex") {
              tabsCreate(`https://translate.yandex.com/translate?view=compact&url=${encodeURIComponent(tabs[0].url)}&lang=${twpConfig.get("targetLanguage").split("-")[0]}`);
            } else {
              // google
              tabsCreate(`https://translate.google.com/translate?tl=${twpConfig.get("targetLanguage")}&u=${encodeURIComponent(tabs[0].url)}`);
            }
          });
          break;
        case "moreOptions":
  ...
  chrome.tabs.query({
    ...
    {
      const text = twpI18n.getMessage("lblShowTranslatedWhenHoveringThisSite");
      if (twpConfig.get("sitesToTranslateWhenHovering").indexOf(hostname) === -1) {
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.1.1/old-popup.js.map
